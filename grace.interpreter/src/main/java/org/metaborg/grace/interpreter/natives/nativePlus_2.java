...
public abstract class nativePlus_2 extends TermBuild {

      public nativePlus_2(SourceSection source) {
              super(source);
      }

      @Specialization
      public int doOp(Object a, Object b) {
    	  if (a instanceof NumV_1_Term && b instanceof NumV_1_Term) {
    		  return ((NumV_1_Term) a).get_1() + ((NumV_1_Term) b).get_1();
    	  } else {
    		  throw new RuntimeException("native_plus only works on integer numbers");
    	  }
      }

      public static TermBuild create(SourceSection source, TermBuild left, TermBuild right) {
              return nativePlus_2NodeGen.create(source, left, right);
      }
}